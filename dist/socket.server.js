import { socketAppHandlers, socketMessagesHandlers, socketCallHandlers, } from './socket/index.js';
let onlineUsersMap = new Map();
export default function (socket, io) {
    // user joins or opens the app
    socket.on('join', socketAppHandlers.joinApp(socket, io, onlineUsersMap));
    // socket is offline
    socket.on('disconnect', socketAppHandlers.disconnect(socket, io, onlineUsersMap));
    // join a conversation room
    socket.on('join conversation', socketMessagesHandlers.joinConversation(socket));
